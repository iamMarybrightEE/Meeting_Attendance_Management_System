...
import { ROLES_DATA, DEPARTMENTS, ACCOUNT_STATUS } from "../../../data/dummyData";
...
export default function UserEditModal({ open, onClose, user, onSuccess }) {
  const { updateUser, changeUserStatus, deleteUser } = useAuth();
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    updateUser(user.id, { ...values, role: role?.name });
    setSubmitting(false);
    onSuccess?.();
    onClose();
  };
      ...
      >
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Chip
            ...
            size="small"
            sx={{ bgcolor: statusStyle.bgcolor, color: statusStyle.color, fontWeight: 600, fontSize: "0.7rem" }}
          />
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
                {user.status !== ACCOUNT_STATUS.ACTIVE && (
                  <Button
                    ...
                    variant="outlined"
                    sx={{ borderRadius: 2, textTransform: "none", borderColor: "#018e11", color: "#018e11", "&:hover": { bgcolor: "#e6f9ee", borderColor: "#018e11" }, justifyContent: "flex-start", px: 2, py: 1.2 }}
                  >
                    Activate Account
                  </Button>
                )}
                {user.status !== ACCOUNT_STATUS.INACTIVE && (
                  <Button
                    startIcon={<PersonOff />}
                    onClick={() => handleStatusChange(ACCOUNT_STATUS.INACTIVE)}
                    variant="outlined"
